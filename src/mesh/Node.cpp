...
void Node::MeshMessageReceivedHandler(BaseConnection* connection, BaseConnectionSendData* sendData, ConnPacketHeader const * packetHeader)
{
    ...
    if(packetHeader->messageType == MessageType::MODULE_TRIGGER_ACTION){
        ...
        if(packet->moduleId == ModuleId::NODE){
            ...
            // 新增：收集混合模式統計（傳送目標發送數，不含重傳）
            else if(packet->actionType == (u8)NodeModuleTriggerActionMessages::COLLECT_MIXED_DATA)
            {
                // 對於隨機模式和混合模式，發送實際生成的HIGH/LOW數量（不含重傳）
                // 這樣sink才能準確計算丟包率
                // PDR = rcvCount / sndCount (不含重傳的目標數量)
                
                // 發送 HIGH priority 目標計數（不含重傳，實際生成的數量）
                SendModuleActionMessage(
                    MessageType::MODULE_TRIGGER_ACTION,
                    packetHeader->sender,
                ...
                // 發送 LOW priority 目標計數（不含重傳，實際生成的數量）
                SendModuleActionMessage(
                    MessageType::MODULE_TRIGGER_ACTION,
                    packetHeader->sender,
                ...
                // 發送 HIGH priority 實際發送數（包含重傳）
                SendModuleActionMessage(
                    MessageType::MODULE_TRIGGER_ACTION,
                    packetHeader->sender,
                    (u8)NodeModuleTriggerActionMessages::TRANSMIT_MIXED_HIGH_ACTUAL,
                    0,
                    (u8*)&generateLoadHighActualSent,
                    sizeof(generateLoadHighActualSent),
                    false
                );
                // 發送 LOW priority 實際發送數（包含重傳）
                SendModuleActionMessage(
                    MessageType::MODULE_TRIGGER_ACTION,
                    packetHeader->sender,
                    (u8)NodeModuleTriggerActionMessages::TRANSMIT_MIXED_LOW_ACTUAL,
                    0,
            ...
            // 接收 HIGH priority 實際發送數（包含重傳）
            else if(packet->actionType == (u8)NodeModuleTriggerActionMessages::TRANSMIT_MIXED_HIGH_ACTUAL)
            {
                u32 count = 0;
                if (sendData->dataLength.GetRaw() >= SIZEOF_CONN_PACKET_MODULE + sizeof(u32)) {
                    CheckedMemcpy(&count, packet->data, sizeof(u32));
                ...
                if (packetHeader->sender > 0 && packetHeader->sender <= 100) {
                    sndCountHighPrioActual[packetHeader->sender - 1] = count;
                }
            }
            // 接收 LOW priority 實際發送數（包含重傳）
            else if(packet->actionType == (u8)NodeModuleTriggerActionMessages::TRANSMIT_MIXED_LOW_ACTUAL)
            {
                u32 count = 0;
                if (sendData->dataLength.GetRaw() >= SIZEOF_CONN_PACKET_MODULE + sizeof(u32)) {
                    CheckedMemcpy(&count, packet->data, sizeof(u32));
                ...
                if (packetHeader->sender > 0 && packetHeader->sender <= 100) {
                    sndCountLowPrioActual[packetHeader->sender - 1] = count;
                }
            }
            //new collect data
...
TerminalCommandHandlerReturnType Node::TerminalCommandHandler(const char* commandArgs[], u8 commandArgsSize)
{
    ...
    if(commandArgsSize >= 3 && TERMARGS(2 , "node"))
    {
        if(TERMARGS(0 ,"action"))
        {
            ...
            {
                trace("\\n========== Priority Statistics (Target Counts - No Retransmissions) ==========" EOL);
                trace("NOTE: snd = TARGET packets generated by random/mixed mode (no retransmit)" EOL);
                trace("      snd_actual = ACTUAL packets sent including retransmissions" EOL);
                trace("      PDR = rcv / snd (accurate packet delivery ratio)" EOL);
                
                u32 avgHigh = 0, avgLow = 0;
                u32 totalHighRcv = 0, totalLowRcv = 0;
                u32 totalHighSnd = 0, totalLowSnd = 0;
                u32 totalHighSndActual = 0, totalLowSndActual = 0;
                ...
                for (int i = 0; i < TOTAL_NODE_NUM; i++)
                {
                    if ((i+1) != destinationNode) {
                        ...
                        // 使用目標發送數（不含重傳）
                        // 對於隨機模式，這是實際隨機生成的 HIGH/LOW 數量
                        // 可以準確計算 PDR = rcv / snd
                        u32 highSendCount = sndCountHighPrio[i];
                        u32 lowSendCount = sndCountLowPrio[i];
                        u32 highSendActual = sndCountHighPrioActual[i];
                        u32 lowSendActual = sndCountLowPrioActual[i];
                        ...
                        totalHighSndActual += highSendActual;
                        totalLowSndActual += lowSendActual;
                        
                        // 計算各節點的 PDR
                        u32 highPDR = highSendCount > 0 ? (rcvCountHighPrio[i] * 100) / highSendCount : 0;
                        u32 lowPDR = lowSendCount > 0 ? (rcvCountLowPrio[i] * 100) / lowSendCount : 0;
                        
                        trace("Node %d: HIGH=%u ms (snd=%u, snd_actual=%u, rcv=%u, PDR=%u%%), LOW=%u ms (snd=%u, snd_actual=%u, rcv=%u, PDR=%u%%)" EOL, 
                            i + 1, 
                            tempHigh, highSendCount, highSendActual, rcvCountHighPrio[i], highPDR,
                            tempLow, lowSendCount, lowSendActual, rcvCountLowPrio[i], lowPDR);
                    }
                }
                ...
                if (activeNodes > 0) {
                    // 計算整體 PDR
                    u32 overallHighPDR = totalHighSnd > 0 ? (totalHighRcv * 100) / totalHighSnd : 0;
                    u32 overallLowPDR = totalLowSnd > 0 ? (totalLowRcv * 100) / totalLowSnd : 0;
                    
                    trace("\\nOverall: HIGH=%u ms (snd=%u, snd_actual=%u, rcv=%u, PDR=%u%%), LOW=%u ms (snd=%u, snd_actual=%u, rcv=%u, PDR=%u%%)" EOL,
                        totalHighRcv > 0 ? avgHigh / activeNodes : 0, totalHighSnd, totalHighSndActual, totalHighRcv, overallHighPDR,
                        totalLowRcv > 0 ? avgLow / activeNodes : 0, totalLowSnd, totalLowSndActual, totalLowRcv, overallLowPDR);
    ...
    else if (TERMARGS(0, "set_config"))
    {
        if (commandArgsSize <= 2) return TerminalCommandHandlerReturnType::NOT_ENOUGH_ARGUMENTS;

        if (TERMARGS(1, "min_conn_iv"))
        {
            u16 value = Utility::StringToU16(commandArgs[2]);
            RamConfig->meshMinConnectionInterval = value;
            logt("CONFIG", "Set meshMinConnectionInterval to %u", value);
            return TerminalCommandHandlerReturnType::SUCCESS;
        }
        else if (TERMARGS(1, "max_conn_iv"))
        {
            u16 value = Utility::StringToU16(commandArgs[2]);
            RamConfig->meshMaxConnectionInterval = value;
            logt("CONFIG", "Set meshMaxConnectionInterval to %u", value);
            return TerminalCommandHandlerReturnType::SUCCESS;
        }
        else if (TERMARGS(1, "scan_iv_high"))
        {
            u16 value = Utility::StringToU16(commandArgs[2]);
            RamConfig->meshScanIntervalHigh = value;
            logt("CONFIG", "Set meshScanIntervalHigh to %u", value);
            return TerminalCommandHandlerReturnType::SUCCESS;
        }
        else if (TERMARGS(1, "scan_iv_low"))
        {
            u16 value = Utility::StringToU16(commandArgs[2]);
            RamConfig->meshScanIntervalLow = value;
            logt("CONFIG", "Set meshScanIntervalLow to %u", value);
            return TerminalCommandHandlerReturnType::SUCCESS;
        }
        return TerminalCommandHandlerReturnType::WRONG_ARGUMENT;
    }
    else if(TERMARGS(0, "update_iv"))     //jstodo can this be removed? Currently untested
    {
        ...
        switch (newConnectionInterval)
        {
            ...
            case MSEC_TO_UNITS(100, CONFIG_UNIT_1_25_MS):
                break;
            case MSEC_TO_UNITS(110, CONFIG_UNIT_1_25_MS):
                break;   
            case MSEC_TO_UNITS(120, CONFIG_UNIT_1_25_MS):
                break;
            case MSEC_TO_UNITS(130, CONFIG_UNIT_1_25_MS):
                break;
            case MSEC_TO_UNITS(140, CONFIG_UNIT_1_25_MS):
                break;
            default:
...
//新加入功能:動態調整connection interval
ErrorType Node::UpdateConnectionInterval(u16 connectionHandle, u16 newIntervalMs)
{
    // Convert interval from milliseconds to BLE units (1.25ms per unit)
    u16 newIntervalUnits = MSEC_TO_UNITS(newIntervalMs, CONFIG_UNIT_1_25_MS);

    // Use default values for slave latency and supervision timeout from Conf
    u16 slaveLatency = Conf::GetInstance().meshPeripheralSlaveLatency;
    u16 supervisionTimeout = Conf::meshConnectionSupervisionTimeout;

    logt("NODE", "Requesting connection parameter update for handle %u: interval %u ms (%u units)",
            connectionHandle, newIntervalMs, newIntervalUnits);

    // Request the update
    ErrorType err = GAPController::GetInstance().RequestConnectionParameterUpdate(
        connectionHandle,
        newIntervalUnits,      // min interval
        newIntervalUnits,      // max interval
        slaveLatency,
        supervisionTimeout
    );

    if (err != ErrorType::SUCCESS && err != (ErrorType)17 /* NRF_ERROR_BUSY */)
    {
        logt("ERROR", "Failed to request connection parameter update: %u", (u32)err);
    }
    return err;
}
