...
// ** Contact: https://www.bluerange.io/licensing
// **
            ...
            if (packet->actionType == 5) { // GENERATE_LOAD_CHUNK
                ...
                if (payloadLength > 0) {
                    ...
                    if ((firstByte & 0xF0) == 0xF0) {
                        u8 priority = firstByte & 0x0F;
                        // 累加到實際發送計數（包含重傳）
                        if (priority <= 1) { // HIGH or VITAL
                            GS->node.generateLoadHighActualSent++;
                        } else { // MEDIUM or LOW
                            GS->node.generateLoadLowActualSent++;
                        }
                    }
                }
            }
