...
void CherrySim::LoadFlashFromFile()
{
    ...
    std::ifstream infile(simConfig.storeFlashToFile, std::ifstream::binary);
    ...
    //If for some reason the file could not be read properly, we throw an exception
    if (!infile.good()) {
...
{ \
    ...
    extern LicenseState GetLicenseState_##featureset(); \
    ...
    fp.getLicenseState = GetLicenseState_##featureset; \
...
void CherrySim::PrepareSimulatedFeatureSets()
{
    ...
    //NOTE 2: The macro for the featureset is defined by the cmakelist in the featureset folder.
...
{
    //Queue interrupts coming from the accelerometer if we are currently moving
    QueueAccelerationInterruptCurrentNode();
}
...
void CherrySim::Init()
{
    ...
    u32 totalSize = GetTotalNodes() * (sizeof(NodeEntry) + alignof(NodeEntry));
    //This will try to allocate a consecutive range of memory to hold the node data
    //If this call fails because of bad_alloc it means that the OS cannot reserve a consecurive range
    //long enough to hold the data. Remember that CherrySim is a 32bit process
    nodeEntryBuffer.resize(totalSize);
    CheckedMemset(nodeEntryBuffer.data(), 0, nodeEntryBuffer.size());
    nodes = (NodeEntry*)nodeEntryBuffer.data();
    for (u32 i = 0; i < GetTotalNodes(); i++)
    {
        currentNode = &nodes[i]; //currentNode must be set as some initializers use the pointer
    ...
    if (simConfig.importFromJson) {
        ImportPositionsAndDataFromJson();
    } else {
...
//Checks if a device entry is importable by the simulator
bool CherrySim::IsUsableDeviceEntry(json deviceJson)
{
    if (
        (
            deviceJson["platform"] == "BLENODE"
            || deviceJson["platform"] == "ASSET"
            || deviceJson["platform"] == "EDGEROUTER"
            )
        &&
        (
            deviceJson["properties"]["onMap"] == true
            || deviceJson["properties"]["onMap"] == "true"
        )
    ) {
        return true;
...
//This will set a number of defaults for an imported json object
nlohmann::json& CherrySim::NormalizeDeviceEntry(nlohmann::json& device)
{
    if (IsUsableDeviceEntry(device))
    {
        //##### Add missing entries and set them to useful defaults

        auto & deviceProperties = device["properties"];

        // Collect property keys which are assigned 'the' null value
        std::vector<std::string> propertiesWithNullValue;
        for (auto & [key, value] : deviceProperties.items())
        {
            if (value.is_null())
            {
                propertiesWithNullValue.emplace_back(key);
            }
        }

        // Delete all these properties
        for (const auto & key : propertiesWithNullValue)
        {
            deviceProperties.erase(key);
        }

        if (!deviceProperties.contains("cherrySimFeatureSet"))
        {
            if (device["platform"] == "EDGEROUTER") deviceProperties["cherrySimFeatureSet"] = "prod_sink_nrf52";
            else if (device["platform"] == "BLENODE") deviceProperties["cherrySimFeatureSet"] = "prod_mesh_nrf52";
            else if (device["platform"] == "ASSET") deviceProperties["cherrySimFeatureSet"] = "prod_asset_nrf52";
            else SIMEXCEPTION(IllegalStateException);
        }

        if (!deviceProperties.contains("zNorm"))
        {
            deviceProperties["zNorm"] = 0;
        }

        if (!deviceProperties.contains("onMap"))
        {
            deviceProperties["onMap"] = false;
        }

        //##### Convert the types accordingly

        if (!deviceProperties["x"].is_number()) {
            deviceProperties["x"] = (float)std::stod(deviceProperties["x"].get<std::string>(), 0);
        }
        if (!deviceProperties["y"].is_number()) {
            deviceProperties["y"] = (float)std::stod(deviceProperties["y"].get<std::string>(), 0);
        }
        if (!deviceProperties["zNorm"].is_number()) {
            deviceProperties["zNorm"] = (float)std::stod(deviceProperties["zNorm"].get<std::string>(), 0);
        }
        if (!deviceProperties["onMap"].is_boolean()) {
            deviceProperties["onMap"] = deviceProperties["onMap"].get<std::string>() == "true";
        }
    }
...
void CherrySim::ImportDataFromJson()
{
    ...
    for (size_t i = 0; i < devicesJson["results"].size(); i++) 
    {
        ...
        {
            json device = NormalizeDeviceEntry(devicesJson["results"][i]);
            auto featuresetAlreadyInserted = simConfig.nodeConfigName.find(device["properties"]["cherrySimFeatureSet"]);

            if (featuresetAlreadyInserted != simConfig.nodeConfigName.end())
            {
                u32 count = featuresetAlreadyInserted->second + 1;
                simConfig.nodeConfigName.insert_or_assign(device["properties"]["cherrySimFeatureSet"], count);
            }
            else
            {
                simConfig.nodeConfigName.insert({ device["properties"]["cherrySimFeatureSet"], 1 });
            }
        }
    }
}
...
//It will also import other data such as serial number, network ids, ....
void CherrySim::ImportPositionsAndDataFromJson()
{
    json devicesJson;
    ...
    for (u32 i = 0; i < devicesJson["results"].size(); i++) 
    {
        if (IsUsableDeviceEntry(devicesJson["results"][i]))
        {
            ...
            //First, we need to find a node with a matching featureset that was not yet configured
            for (u32 j = 0; j < GetTotalNodes(); j++) {
                if (!nodes[j].jsonDataImported && nodes[j].nodeConfiguration == device["properties"]["cherrySimFeatureSet"].get<std::string>()) {
                    nodeIndex = j;
                    break;
                }
            }
            //Could not properly match node entries, implementation error
            if (nodeIndex == -1) SIMEXCEPTION(IllegalStateException);

            //Import position
            nodes[nodeIndex].x = device["properties"]["x"];
            nodes[nodeIndex].y = device["properties"]["y"];
            nodes[nodeIndex].z = device["properties"]["zNorm"];

            //Import Serial Number as SerialNumberIndex
            if (device.contains("deviceId")) {
                const u32 configuredSerialNumberIndex = Utility::GetIndexForSerial(device["deviceId"].get<std::string>().c_str(), nullptr);
            ...
            if (device["properties"].contains("IOT_NODE_KEY")) {
                ...
                nodes[nodeIndex].uicr.CUSTOMER[4] = Utility::ByteFromAsciiHex(chars, 8);
                nodes[nodeIndex].uicr.CUSTOMER[5] = Utility::ByteFromAsciiHex(chars + 8, 8);
                nodes[nodeIndex].uicr.CUSTOMER[6] = Utility::ByteFromAsciiHex(chars + 16, 8);
            ...
            GenerateLicense(nodeIndex);
        }
        else {
            printf("Skipped one device entry as it could not be parsed" EOL);
        }
    }
}
...
void HelperPositionNodesRandomly(CherrySim &instance, std::vector<point_t> &points, u32 numberOfNodesToPlace)
{
    ...
    double epsilon = pow(10, ((double)-STABLE_CONNECTION_RSSI_THRESHOLD + SIMULATOR_NODE_DEFAULT_CALIBRATED_TX + SIMULATOR_NODE_DEFAULT_DBM_TX) / 10 / instance.propagationConstant);
...
void CherrySim::PositionNodesRandomly()
{
    ...
    for (u32 i = 0; i < totalNodes; i++) {
        ...
        nodes[i].x = (col + 1) * spacingX;  // X 坐標（列）
        nodes[i].y = (row + 1) * spacingY;  // Y 坐標（行）
        nodes[i].z = 0;
    }
...
void CherrySim::LoadPresetNodePositions()
{
    if (simConfig.preDefinedPositions.size() != 0)
    {
        ...
        {
            nodes[nodeIndex].x = (float)simConfig.preDefinedPositions[nodeIndex].x;
            nodes[nodeIndex].y = (float)simConfig.preDefinedPositions[nodeIndex].y;
            nodes[nodeIndex].z = (float)simConfig.preDefinedPositions[nodeIndex].z;
        }
    }
}
...
{
    //loggedExceptions are cleared at the start of a simulation step
    ...
    while (meshGwCommunication && !receivedDataFromMeshGw)
    {
        ...
        // Process available events of the socket-based communication. This is required because otherwise the
    ...
    for (u32 i = 0; i < GetTotalNodes(); i++) {
        ...
        // Update the cached floor index using the current z-coordinate of the node
        currentNode->currentFloorNumber = FindFloorNumber(currentNode->z);
    ...
    for (u32 i = 0; i < GetTotalNodes(); i++) {
        ...
        {
            const int64_t nodeSimualtedFramesBelowAverage = avgSimulatedFrames - currentNode->simulatedFrames;
            // Sigmoid function, flipped on the Y-Axis.
            const double probabilityToSkipNodeSimulation = 1.0 / (1 + std::exp((double)(nodeSimualtedFramesBelowAverage) * 0.1));
            if (PSRNG(probabilityToSkipNodeSimulation * UINT32_MAX))
            {
                simulateNode = false;
            }
        }
        ...
        if (simulateNode)
        {
            ...
            SimulateTimeslot();
            SimulateConnectionParameterUpdateRequestTimeout();
            try {
...
#endif
    //Call all sim step handlers that were registered
    for (auto & callback : simStepCallbacks)
    {
        callback();
    }
}
...
void LogThrownCherrySimException(std::type_index index)
{
    if (cherrySimInstance == nullptr)
    {
        return;
    }
    cherrySimInstance->LogThrownException(index);
}
...
{
    if (loggedExceptions.find(index) != loggedExceptions.end()) {
        return true;
    }
    return false;
}
...
TerminalCommandHandlerReturnType CherrySim::TerminalCommandHandler(const std::vector<std::string>& commandArgs)
{
    if (commandArgs.size() >= 2 && commandArgs[0] == "sim")
    {
        if (commandArgs[1] == "stat" || commandArgs[1] == "help") {
            ...
            // processed separately (i.e. this if branch will _not_ be called if 'sim term ...' is issued on a
            // socket client).
...
void CherrySim::TerminalPrintHandler(const char* message)
{
    ...
    if (terminalPrintListener != nullptr) {
        ...
        if (currentNode == nullptr || IsSimTermOfCurrentNodeActive()) {
...
void CherrySim::SetNode(u32 i)
{
    ...
    __license_data_start_address = __application_start_address + LICENSE_APP_IV_OFFSET;
...
void CherrySim::SetFeaturesets()
{
    ...
    {
        featureset = it->first;
        auto entry = cherrySimInstance->featuresetPointers.find(featureset);
        if (entry == cherrySimInstance->featuresetPointers.end())
        {
            if (it->first == "prod_sink_nrf52")
            {
        ...
        pair.featuresetName = featureset;
...
void CherrySim::WriteSerialNumberToUicr(const char * serialNumber, u32 nodeIndex)
{
    u32 len = (u32)strlen(serialNumber);
    // Make sure it will fit into UICR
    if (len > NODE_SERIAL_NUMBER_MAX_CHAR_LENGTH) {
    ...
    nodes[nodeIndex].uicr.CUSTOMER[12] = serialNumberIndex;
}

void CherrySim::WriteSerialNumberToUicr(u32 serialNumberIndex, u32 nodeIndex)
{
    Utility::GenerateBeaconSerialForIndex(serialNumberIndex, (char*)(nodes[nodeIndex].uicr.CUSTOMER + 2));

    nodes[nodeIndex].uicr.CUSTOMER[12] = serialNumberIndex;
}

// Dummy implementations for licensing as we only use firmware licenses for specific commercial
// BlueRange firmware releases and do not include this functionality in FruityMesh
#ifdef GITHUB_RELEASE
...
void CherrySim::FlashNode(u32 i) {
    ...
    nodes[i].ficr.DEVICEID[0] = simState.rnd.NextU32();
    nodes[i].ficr.DEVICEID[1] = simState.rnd.NextU32();
    ...
    //This is basically deprecated, but we have some legacy nodes flashed with this value
    if (GET_DEVICE_TYPE() == DeviceType::ASSET) nodes[i].uicr.CUSTOMER[11] = (u32)DeviceType::ASSET;

    // writes serialNumberIndex and serialNumber
    WriteSerialNumberToUicr(i, i);
    ...
    *((u32*)&nodes[i].flash[currentNode->uicr.BOOTLOADERADDR + 1024]) = 123;
    ...
    //TODO: Currently, we only support one specific SoftDevice per chipset
    //This should be refactored to store the used BLE Stack as part of the featureset
    if (GetChipset_CherrySim() == Chipset::CHIP_NRF52) {
        nodes[i].bleStackType = BleStackType::NRF_SD_132_ANY;
    }
    else if (GetChipset_CherrySim() == Chipset::CHIP_NRF52840) {
        nodes[i].bleStackType = BleStackType::NRF_SD_140_ANY;
    }

    if (nodes[i].bleStackType == BleStackType::NRF_SD_132_ANY) {
        CheckedMemcpy(simFlashPtr, s132_mbr_and_header_nrf52_v5_1_0, sizeof(s132_mbr_and_header_nrf52_v5_1_0));
    }
    else if (nodes[i].bleStackType == BleStackType::NRF_SD_140_ANY) {
        CheckedMemcpy(simFlashPtr, s140_mbr_and_header_nrf52840_v6_1_0, sizeof(s140_mbr_and_header_nrf52840_v6_1_0));
    }
...
void CherrySim::BootCurrentNode()
{
    ...
    //Executes some minimalistic Bootloader simulation for testing updates
    SimulateFruityLoader();
    ...
    new (&(currentNode->eventQueue)) std::deque<simBleEvent>();
...
void CherrySim::ResetCurrentNode(RebootReason rebootReason, bool throwException, bool powerLoss) {
    ...
    //If a power loss should be simulated, we clear the retained RAM
    if (powerLoss) {
        currentNode->retainedRamMemory = {};
    }
...
// Simulation of FruityLoader Bootloader
// Provides a simplistic simulation of the FruityLoader that can be used to test updates
//#########################################################################################

void CherrySim::SimulateFruityLoader()
{
    ...
    {
        printf("FruityLoader on node index %u: Magic Number found, updating...." EOL, currentNode->index);

        if (settings->imageType == ImageType::APP || settings->imageType == ImageType::SD_AND_APP)
        {
            u32 dstStartPage = 0;
            if (settings->imageType == ImageType::APP) dstStartPage = settings->appStartPage;
            else if (settings->imageType == ImageType::SD_AND_APP) dstStartPage = settings->sdStartPage;
            else SIMEXCEPTION(IllegalStateException);

            //Clear all destination pages
            for (u32 i = 0; i < settings->imageNumPages; i++) {
                const u32 pageAddr = FLASH_REGION_START_ADDRESS + dstStartPage * FruityHal::GetCodePageSize();
                ErasePage(pageAddr);
            }

            //Copy the application pages
            for (u32 i = 0; i < settings->imageNumPages; i++) {
                const u32 srcAddr = FLASH_REGION_START_ADDRESS + settings->imageStartPage * FruityHal::GetCodePageSize();
                const u32 dstAddr = FLASH_REGION_START_ADDRESS + dstStartPage * FruityHal::GetCodePageSize();

                CheckedMemcpy((u32*)dstAddr, (u32*)srcAddr, FruityHal::GetCodePageSize());
            }

            //Erase the Bootloader Settings Page after the update was finished
            ErasePage(settingsPageAddress);
        }
        else
        {
            //Right now, we only have simulation for APP + APP_AND_SD updates implemented
            SIMEXCEPTION(NotImplementedException)
        }
    }
    else if (settings->updatePending != 0xFFFFFFFFL && settings->updatePending != 0x00) {
        //The magic number should not contain any other value
        SIMEXCEPTION(IllegalStateException);
    }

    //Now, the bootloader would start the application, we can do some basic checks here to
    //make sure we have a valid application
    //TODO: E.g. check version number of this FruityMesh application
...
void CherrySim::SimulateAdvertising() {
    ...
    if (currentNode->state.advertisingActive) {
        if (ShouldSimIvTrigger(currentNode->state.advertisingIntervalMs)) {
            const u32 indexStep = std::max<u32>(simConfig.simulateAdvertisingIndexStep, 1);
            const u32 startIndex = (indexStep == 1 ? 0 : simState.rnd.NextU32() % indexStep);
            const u32 nodeCount = GetTotalNodes() - GetAssetNodes();

            //Distribute the event to all nodes in range
            for (u32 i = startIndex; i < nodeCount; i += indexStep) {
                if (i != currentNode->index) {
                    //If the random value hits the probability, the event is sent
                    const uint32_t probability = [this, indexStep, i] {
                        const uint32_t rawProbability = CalculateReceptionProbabilityForAdvertisement(currentNode, &nodes[i]);
                        if (simConfig.perfectReceptionProbabilityForAdvertising) {
                            return rawProbability;
                        }
                        else {
                            const uint32_t scaledProbability = indexStep * rawProbability;
                            // Catch overflow (i.e. if the base probability was large enough that the
                            // increase to balance our the index step took the probability over 1)
                            if (scaledProbability / indexStep != rawProbability)
                            {
                                SIMEXCEPTIONFORCE(IllegalStateException);
                            }
...
void CherrySim::ConnectMasterToSlave(NodeEntry* master, NodeEntry* slave)
{
    ...
    if (this->simConfig.verbose) {
        json j1;
        j1["type"] = "sim_connect";
        j1["nodeId"] = master->GetNodeId();
        j1["partnerId"] = slave->GetNodeId();
...
u32 CherrySim::DisconnectSimulatorConnection(SoftdeviceConnection* connection, u32 hciReason, u32 hciReasonPartner) {
    ...
    if (this->simConfig.verbose)
    {
        ...
        j["nodeId"] = connection->owningNode->GetNodeId();
        j["partnerId"] = partnerNode->GetNodeId();
...
void CherrySim::GenerateWrite(SoftDeviceBufferedPacket* bufferedPacket) {
    ...
    if (this->simConfig.verbose)
    {
        json j;
        j["type"] = "sim_data";
        j["nodeId"] = sender->GetNodeId();
        j["partnerId"] = receiver->GetNodeId();
...
void CherrySim::SimulateBatteryUsage()
{
    ...
    u32 conn100Ms = 150 * 1000 / divider; //150 uA per connection at 100ms interval
    ...
    for (u32 i = 0; i < GetSafeConfiguredTotalConnectionCount(currentNode); i++) {
        ...
        if (conn->connectionActive) {
            if (conn->connectionInterval == 100) {
             ...
             else if (conn->connectionInterval == 20) {
                currentNode->nanoAmperePerMsTotal += conn30Ms;
            }
            else if (conn->connectionInterval == 30) {
                currentNode->nanoAmperePerMsTotal += conn30Ms;
            }
            else if (conn->connectionInterval == 50) {
                currentNode->nanoAmperePerMsTotal += conn90Ms;
            }
            else if (conn->connectionInterval == 90) {
                currentNode->nanoAmperePerMsTotal += conn90Ms;
            }
            else if (conn->connectionInterval == 110) {
                currentNode->nanoAmperePerMsTotal += conn100Ms; // 先借用100ms的耗電模型
            }
            else if (conn->connectionInterval == 120) {
                currentNode->nanoAmperePerMsTotal += conn100Ms;
            }
            else if (conn->connectionInterval == 130) {
                currentNode->nanoAmperePerMsTotal += conn100Ms;
            }
...
void CherrySim::SimulateConnectionParameterUpdateRequestTimeout()
{
    for (u32 connIndex = 0; connIndex < GetSafeConfiguredTotalConnectionCount(currentNode); ++connIndex)
    {
        ...
        // Skip connections where we are not the central.
        if (!connection.isCentral)
        {
            continue;
        }
        // Skip connections where no connection parameter update request is pending.
        if (!connection.connParamUpdateRequestPending)
        {
            continue;
        }
        // Skip connections that have not timed out yet.
        if (currentNode->gs.appTimerDs < connection.connParamUpdateRequestTimeoutDs)
        {
            continue;
        }

        // After timing out the request is not pending anymore.
        connection.connParamUpdateRequestPending = false;

        // The request has timed out, generate an event on the _peripheral_ with
        // the current connection parameters (i.e. similar to when a request
        // is rejected).
        auto & peripheral = *connection.partner;
        const auto & peripheralConnection = *connection.partnerConnection;
        ...
        bleEvent.evt.gap_evt.conn_handle = peripheralConnection.connectionHandle;
...
void CherrySim::CheckMeshingConsistency()
{
    ...
    for (u32 i = 0; i < numNoneAssetNodes; i++)
    {
        ...
        {
            printf(
                "NODE %d has a real cluster size of %d and predicted size of %d, reported cluster size %d" EOL,
                node->GetNodeId(), realClusterSize, node->state.validityClusterSize, nodes[i].gs.node.GetClusterSize());
...
MeshConnectionBond findBond(NodeEntry* startNode, NodeEntry* partnerNode)
{
    ...
    for (int i = 0; i < conns.count; i++) {
        if (conns.handles[i].IsHandshakeDone() && conns.handles[i].GetPartnerAddress() == partnerNode->address) {
    ...
    for (int i = 0; i < partnerConns.count; i++) {
        if (partnerConns.handles[i].IsHandshakeDone() && partnerConns.handles[i].GetPartnerAddress() == startNode->address) {
...
ClusterSize CherrySim::DetermineClusterSizeAndPropagateClusterUpdates(NodeEntry* node, NodeEntry* startNode)
{
    ...
    for (int i = 0; i < conns.count; i++)
    {
        ...
        NodeEntry* nextPartner = FindUniqueNodeById(conn->partnerId, node->GetNetworkId());
...
LicenseState GetLicenseState_CherrySim()
{
    return getFeaturesetPointers()->getLicenseState();
}
...
void CherrySim::SetBleStack(NodeEntry* node)
{
    ...
    else if (node->bleStackType == BleStackType::NRF_SD_140_ANY) {
        //TODO: These values have only been copied from the S132 and
        //no different simulation is done here
        node->bleStackMaxPeripheralConnections = 10;
        node->bleStackMaxCentralConnections = 10;
        node->bleStackMaxTotalConnections = 10;
    }
...
{
    LambdaWithHandle data;
    data.owningNode = currentNode;
    data.lambda = lambda;
    simStepCallbacks.push_back(data);
}

void CherrySim::CleanSimulationStepHandlers(NodeEntry* nodeEntry)
{
    auto it = std::remove_if(simStepCallbacks.begin(), simStepCallbacks.end(),
        [nodeEntry](const auto& entry) {
            return nodeEntry == entry.owningNode;
        }
    );

    if (it != simStepCallbacks.end())
    {
        simStepCallbacks.erase(it, simStepCallbacks.end());
    }
}
...
void CherrySim::ChooseSimulatorTerminal() {
    ...
    if (IsSimTermOfCurrentNodeActive()) {
...
{
    // Early out if the nodes are too far from each other to optimize the performance for bigger scenarios
    if (    abs(sender->x - receiver->x) * simConfig.mapWidthInMeters > 50
        ||  abs(sender->y - receiver->y) * simConfig.mapHeightInMeters > 50
        ||  abs(sender->z - receiver->z) * simConfig.mapElevationInMeters > 50)
    {
        return -1000;
    }

    // The RSSI must be clamped to below -11, because the AssetScanningModule is filtering out received
    // advertisements with RSSIs above -10 (on real hardware such measurements are invalid). In the simulator
    // however (due to the nature of the computation) the RSSIs for nodes that are extremely close (or even
    // in the same location) will be better than -10, which would falsely lead to all advertisements being
    // rejected.
    const auto clampRssi = [] (const float rssi) { return Utility::Clamp<float>(rssi, std::numeric_limits<float>::lowest(), -11.0f); };

    // Compute the RSSI for transmissions between the two nodes
    const float rssi = GetReceptionRssiNoNoise(sender, receiver);
    ...
    // Generate RSSI noise with the specified parameters
    const float noise = GenerateRssiNoise(simState.rnd, rssiNoiseStddev, rssiNoiseMean);

    return clampRssi(rssi + noise);
}

float CherrySim::GetReceptionRssiNoNoise(const NodeEntry *sender, const NodeEntry *receiver)
{
    // If either the sender or the receiver has the other marked as a impossibleConnection, the rssi is set to a unconnectable level.
    if (!sender->impossibleConnection.empty() || !receiver->impossibleConnection.empty())
    {
    ...
    const float distance = GetDistanceBetween(sender, receiver);

    const PathLossModelParameters parameters = {
        .receivedPowerAtReferenceDistanceDbm = static_cast<float>(sender->gs.config.defaultDBmTX) + static_cast<float>(sender->gs.boardconf.configuration.calibratedTX),
        .propagationConstant                 = propagationConstant,
    };
    const float rssiFromDistance = ComputeRssiFromDistance(distance, parameters);

    const auto ceilingAttenuation = std::abs(static_cast<float>(sender->currentFloorNumber - receiver->currentFloorNumber) * simConfig.ceilingAttenuationDb);

    return rssiFromDistance - ceilingAttenuation;
}

uint32_t CherrySim::CalculateReceptionProbabilityFromRssi(const float rssi)
{
      if      (rssi < -100) return 0;
      else if (rssi > -60)  return simConfig.receptionProbabilityVeryClose;
...
uint32_t CherrySim::CalculateReceptionProbabilityForConnection(const NodeEntry *sendingNode, const NodeEntry *receivingNode)
{
    const auto rssiProbability = CalculateReceptionProbabilityFromRssi(GetReceptionRssi(sendingNode, receivingNode));

    if (simConfig.perfectReceptionProbabilityForConnection && rssiProbability > 0)
    {
        return UINT32_MAX;
    }

    return rssiProbability;
}

uint32_t CherrySim::CalculateReceptionProbabilityForAdvertisement(const NodeEntry *sendingNode, const NodeEntry *receivingNode)
{
    // If the scanIntervalMs is 0, scanning is effectively disabled (i.e. no reception possible)
    const auto scanIntervalMs = receivingNode->state.connectingActive ? receivingNode->state.connectingIntervalMs : receivingNode->state.scanIntervalMs;
    if (scanIntervalMs == 0)
    {
        return 0;
    }
    ...
    if (simConfig.perfectReceptionProbabilityForAdvertising && rssiProbability > 0)
    {
        return UINT32_MAX;
    }

    const auto scanWindowMs = receivingNode->state.connectingActive ? receivingNode->state.connectingWindowMs : receivingNode->state.scanWindowMs;

    // The scan window (i.e. the time the node is scanning per interval) can not be greater than
    // the scan interval (i.e. the time between scan windows).
    if (scanWindowMs > scanIntervalMs)
    {
        SIMEXCEPTIONFORCE(IllegalStateException);
    }

    const float dutyCycle   = static_cast<float>(scanWindowMs) / static_cast<float>(scanIntervalMs);
    const auto  probability = static_cast<uint32_t>(std::floor(dutyCycle * static_cast<float>(rssiProbability)));
...
bool CherrySim::IsSimTermOfCurrentNodeActive() const
{
    ...
    return simConfig.terminalId == 0 || static_cast<TerminalId>(simConfig.terminalId) == currentNode->GetTerminalId();
}
...
NodeEntry *CherrySim::FindUniqueNodeById(NodeId nodeId)
{
    NodeEntry *nodeEntry = nullptr;
    for (u32 index = 0; index < GetTotalNodes(); index++)
    {
        if (nodes[index].GetNodeId() == nodeId)
        {
            if (nodeEntry != nullptr)
            {
                SIMEXCEPTIONFORCE(MulipleNodesHaveSameNodeIdException);
            }
...
NodeEntry* CherrySim::FindUniqueNodeById(NodeId nodeId, NetworkId networkId)
{
    NodeEntry *nodeEntry = nullptr;
    for (u32 index = 0; index < GetTotalNodes(); index++)
    {
        auto &currentNodeEntry = nodes[index];
        if (currentNodeEntry.GetNodeId() == nodeId && currentNodeEntry.GetNetworkId() == networkId)
        {
            if (nodeEntry != nullptr)
            {
                SIMEXCEPTIONFORCE(MulipleNodesHaveSameNodeAndNetworkIdException);
            }
...
NodeEntry* CherrySim::FindUniqueNodeByTerminalId(TerminalId terminalId)
{
    const auto nodeIndex = terminalId - 1;
    if (nodeIndex > GetTotalNodes())
    {
        SIMEXCEPTION(InvalidTerminalIdException);
        return nullptr;
    }

    NodeEntry * const nodeEntry = &nodes[nodeIndex];
    if (nodeEntry->GetTerminalId() != terminalId)
    {
        SIMEXCEPTIONFORCE(IllegalStateException);
    }
...
void CherrySim::PrintPacketStats(NodeId nodeId, const char* statId)
{
    ...
    //We simply select the stat from the given nodeId
        ...
        NodeEntry* node = FindUniqueNodeById(nodeId);
...
bool CherrySim::IsValidMoveAnimationJson(const nlohmann::json &json) const
{
    ...
    for (nlohmann::json::const_iterator it = json.begin(); it != json.end(); ++it)
    {
        ...
        for (nlohmann::json::const_iterator it2 = it.value()["key_points"].begin(); it2 != it.value()["key_points"].end(); ++it2)
        {
            ...
            if (!it2.value().contains("zNorm"))
            {
                printf("Key Point in Animation %s did not contain a zNorm value!" EOL, it.key().c_str());
                foundErrorInJson = true;
            }
            else if (!IsNumberType(it2.value()["zNorm"].type()))
            {
...
bool CherrySim::AnimationLoadJsonFromPath(const char * path)
{
    ...
    for (nlohmann::json::const_iterator it = json.begin(); it != json.end(); ++it)
    {
        ...
        for (nlohmann::json::const_iterator it2 = it.value()["key_points"].begin(); it2 != it.value()["key_points"].end(); ++it2)
        {
            ...
            {
                AnimationAddKeypoint(it.key(), it2.value()["x"], it2.value()["y"], it2.value()["zNorm"], it2.value()["durationSec"], StringToMoveAnimationType(it2.value()["type"]));
            }
            else
            {
                AnimationAddKeypoint(it.key(), it2.value()["x"], it2.value()["y"], it2.value()["zNorm"], it2.value()["durationSec"]);
            }
        }
    }
...
i8 CherrySim::FindFloorNumber(float zNorm)
{
    // Sanitize the ceiling height as it is used as a divisor
    if (simConfig.ceilingHeightInMeters < 0.01f)
    {
        SIMEXCEPTIONFORCE(IllegalStateException);
    }

    // Compute the floor number and always round it down
    const auto floorNumber = std::floor((zNorm * simConfig.mapElevationInMeters - simConfig.floorBiasInMeters) / simConfig.ceilingHeightInMeters);

    if (floorNumber < INT8_MIN || floorNumber > INT8_MAX)
    {
        SIMEXCEPTIONFORCE(IllegalArgumentException);
    }

    return static_cast<i8>(floorNumber);
}
