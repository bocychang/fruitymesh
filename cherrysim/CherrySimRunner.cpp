...
SimConfiguration CherrySimRunner::CreateDefaultSimConfiguration()
{
    ...
    simConfig.nodeConfigName.insert({ "prod_mesh_nrf52", 16});//set Dev node, can be multiple nodes
...
void CherrySimRunner::Init()
{
    ...
    // Set different connection and scan intervals for each node AFTER boot
    // and refresh the scanner to apply the new settings.
    // if (sim->GetTotalNodes() >= 3) {
    //     // Node 1 (0-indexed)
    //     sim->nodes[0].gs.config.meshMinConnectionInterval = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_1_25_MS);
    //     sim->nodes[0].gs.config.meshMaxConnectionInterval = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_1_25_MS);
    //     sim->nodes[0].gs.config.meshScanIntervalHigh = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_0_625_MS);
    //     sim->nodes[0].gs.config.meshScanIntervalLow = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_0_625_MS);
    //     sim->nodes[0].gs.node.ChangeState(DiscoveryState::IDLE);
    //     sim->nodes[0].gs.node.ChangeState(DiscoveryState::HIGH);

    //     // Node 2 (1-indexed)
    //     sim->nodes[1].gs.config.meshMinConnectionInterval = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_1_25_MS);
    //     sim->nodes[1].gs.config.meshMaxConnectionInterval = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_1_25_MS);
    //     sim->nodes[1].gs.config.meshScanIntervalHigh = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_0_625_MS);
    //     sim->nodes[1].gs.config.meshScanIntervalLow = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_0_625_MS);
    //     sim->nodes[1].gs.node.ChangeState(DiscoveryState::IDLE);
    //     sim->nodes[1].gs.node.ChangeState(DiscoveryState::HIGH);

    //     // Node 3 (2-indexed)
    //     sim->nodes[2].gs.config.meshMinConnectionInterval = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_1_25_MS);
    //     sim->nodes[2].gs.config.meshMaxConnectionInterval = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_1_25_MS);
    //     sim->nodes[2].gs.config.meshScanIntervalHigh = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_0_625_MS);
    //     sim->nodes[2].gs.config.meshScanIntervalLow = (u16)MSEC_TO_UNITS(100, CONFIG_UNIT_0_625_MS);
    //     sim->nodes[2].gs.node.ChangeState(DiscoveryState::IDLE);
    //     sim->nodes[2].gs.node.ChangeState(DiscoveryState::HIGH);
    // }
